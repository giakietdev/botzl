...
des = {
    'version': "1.1.0",
    'credits': "QuangThang x TRBAYK❄️ ",
    'description': "Kick member from group <user_id or tag or reply>",
    'quyen su dung': "admin box , admin bot"
}

def handle_kick_user_command(message, message_object, thread_id, thread_type, author_id, client):
    text = message.split()
    ...
    if message_object.mentions:
        ...
        if len(text) < 2:
            error_message = Message(text="Vui lòng sử dụng cú pháp: .kick @tag hoặc .kick <ID>.")
            client.sendMessage(error_message, thread_id, thread_type)
            return
        user_id = text[1]
